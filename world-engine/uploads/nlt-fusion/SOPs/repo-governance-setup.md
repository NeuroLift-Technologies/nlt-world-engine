...
1. Repo-local governance (private, primary):
   https://github.com/NeuroLift-Technologies/nlt-fusion/blob/main/NLT-DEV-OTOI.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/nlt-fusion/blob/main/governance/NLT-DEV-OTOI.md
2. Internal gateway (private, primary):
   https://github.com/NeuroLift-Technologies/nlt-fusion/blob/main/AGENTS.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/nlt-fusion/blob/main/governance/AGENTS.md
...
Format reference: `NeuroLift-Technologies/nlt-fusion` templates directory.
```
...
Format reference: `templates/escalation.md` in `NeuroLift-Technologies/nlt-fusion`.
```

### Step 5: Grant GitHub App Access to `nlt-fusion`

Coding agents (e.g., GitHub Copilot, Codex) must be able to read the governance files in
`nlt-fusion` from within the new repository. If the GitHub App installation is scoped
to **"Selected repositories"**, you must add `nlt-fusion` to its access list.
...
3. Under **Repository access → Selected repositories**, add **`nlt-fusion`**.
4. Click **Save**.
...
> in `nlt-fusion` for full instructions.
...
- [ ] GitHub App has access to `nlt-fusion` (or public mirror fallback is in place)

---

## Validation

Run the org-level validation script to check `nlt-fusion` is healthy:

```bash
git clone https://github.com/NeuroLift-Technologies/nlt-fusion
cd nlt-fusion
