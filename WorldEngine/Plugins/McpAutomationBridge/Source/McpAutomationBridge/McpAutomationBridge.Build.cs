...
public class McpAutomationBridge : ModuleRules {
    ...
    public McpAutomationBridge(ReadOnlyTargetRules Target) : base(Target) {
        ...
        if (Target.bBuildEditor) {
            ...
            // UE5.8: UnrealEditor-PCG.dll exports a data symbol (PCG::Private::UserParameterTagData),
            // which cannot be delay-loaded (LNK1194). Link PCG normally instead of delay-loading it.
            if (bHasPCG) {
                PublicDelayLoadDLLs.Remove("UnrealEditor-PCG.dll");
                Console.WriteLine("McpAutomationBridge: PCG linked without delay-load (LNK1194 workaround)");
            }
